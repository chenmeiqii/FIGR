...
class SandboxFusionTool(BaseTool):
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[str, float, dict]:
        ...
        if isinstance(result, dict):
            if result["stderr"] == "":
                if result.get("api_response", {}).get("files", {}).get("image.txt", "") != "":
                    try:
                        outer_b64 = result["api_response"]["files"]["image.txt"]
                        if not outer_b64.strip():
                            self._instance_dict[instance_id]["reward"].append(-0.05)
                            return ToolResponse(text="Base64 string is empty"), -0.05, {"success": False}
                        inner_text = base64.b64decode(outer_b64).decode("utf-8")
                        # print("/===============inner_text", f"inner_text[:100]:{inner_text[:100]}...", "===============inner_text/")
                        images_result = [line.strip() for line in inner_text.splitlines() if line.strip()]
                    except ValueError as e:
                        self._instance_dict[instance_id]["reward"].append(-0.05)
                        return ToolResponse(text=f"Error decoding base64: {str(e)}"), -0.05, {"success": False}
                    except UnicodeDecodeError as e:
                        self._instance_dict[instance_id]["reward"].append(-0.05)
                        return ToolResponse(text=f"Error decoding utf-8 text: {str(e)}"), -0.05, {"success": False}
                    except Exception as e:
                        self._instance_dict[instance_id]["reward"].append(-0.05)
                        return ToolResponse(text=f"Unexpected error: {str(e)}"), -0.05, {"success": False}
            else:
                self._instance_dict[instance_id]["reward"].append(-0.05)
        ...
        imgs = [process_image({"image":f"data:image/jpeg;base64,{img}"}) for img in images_result]

        if result["stdout"].strip() == "":
            text_result = None
        else:
            text_result = result["stdout"].strip()
        if len(imgs) == 0 and text_result:
            if len(code) > 100:
                self._instance_dict[instance_id]["reward"].append(0.1)
                return ToolResponse(text=f"Text Result: {text_result}, Image Result: None"), 0.1, {"success": True}
            else:
                self._instance_dict[instance_id]["reward"].append(-0.05)
                return ToolResponse(text=f"Text Result: {text_result}, Image Result: None"), -0.05, {"success": True}
        elif len(imgs) == 0 and not text_result:
            self._instance_dict[instance_id]["reward"].append(-0.05)
            return ToolResponse(text="No stdout or images to show here, remember to use print() or plt.show()"), -0.05, {"success": False}   
        elif len(imgs) == 1:
            self._instance_dict[instance_id]["reward"].append(0.2)
            return ToolResponse(text=f"Text Result: {text_result}, Image Result: as shown in the figure.", image=imgs), 0.2, {"success": True}
        elif len(imgs) > 1:
            self._instance_dict[instance_id]["reward"].append(0.1)
            return ToolResponse(text=f"Text Result: {text_result}, Image Result: as shown in the figure. We can just display one image at one call", image=[imgs[0]]), 0.1, {"success": True}
