...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    acc_scores, format_scores, tool_scores = [], [], []
    for info in reward_extra_info:
        acc_scores.append(info["acc_score"])
        format_scores.append(info["format_score"])
        tool_scores.append(info["tool_score"])
    if len(acc_scores) > 0:
        metrics["scores/acc_scores/mean"] = np.mean(acc_scores)
    if len(format_scores) > 0:
        metrics["scores/format_scores/mean"] = np.mean(format_scores)
    if len(tool_scores) > 0:
        metrics["scores/tool_scores/mean"] = np.mean(tool_scores)
